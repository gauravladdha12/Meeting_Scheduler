...
class main{
    static int meeting_id = 0; // for generating meeting id
    static HashMap<Integer,ArrayList<slot>> room=new HashMap<>(); // for storing the time slots in rooms
...
static boolean cancel(HashMap<Integer,ArrayList<meeting>> hm,int cancel_id,int emp_id){
	...
	public static void main(String[] args) {
        ...
        o: while (sc.hasNextLine()) {
            ...
            System.out.println();
            String string_in[]=sc.nextLine().split(" ");
            if(string_in.length==4&&string_in[0].compareTo("book")==0){
            
		int emp_id=Integer.parseInt(string_in[1]);
        if(emp_id>n){
            System.out.println("Employee Id is not valid");
            continue o;
        }
        // extracting data required from the input
		String start_time=string_in[2];
        String end_time=string_in[3];
       ...
       if(betweenDates(d_start,d_end)!=0){
        ...
        // checing the room that can be booked 
        // c stores the room id if can be booked else -1
        ...
        // if can book room blocking time slot
        ...
        // storing the total current meeting for partiular employee for checking the constraints
    ...
    else if(string_in.length==3&&string_in[0].compareTo("cancel")==0){
        int cancel_id=Integer.parseInt(string_in[2]);
        int cancel_emp_id=Integer.parseInt(string_in[1]);

        // cancel function return if we can cancel or not

        boolean c=cancel(hm,cancel_id,cancel_emp_id);
        if(c){
            // reducing total meeting for particular employee
